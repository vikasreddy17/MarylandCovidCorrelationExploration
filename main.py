...
def user_file_down(maryland_covid_data):
  correlation_matrix = maryland_covid_data.corr()
  sea.heatmap(correlation_matrix, annot=True)
  plt.savefig('If_Program_Used_User_Dowloads_Files/USERCOPY_corr_heatmap_maryland_covid.png')
...
#first interaction with the program to start off
exist_data = None
intro_info()
fea_ran_list = []
cleaned_correlation_matrix = imp_clean_corr_data()
selected_feature = menu_screen_select_fea()
exist_data = create_print_rows(selected_feature, cleaned_correlation_matrix)
fea_ran_list.append(selected_feature)
print(exist_data)
time.sleep(2)

#continues the program after the first interaction
l = 0
for try_again_loop in range(6):
  l = l+1
  are_done = ask_to_try_again()
  if are_done == False:
    cleaned_correlation_matrix = imp_clean_corr_data()
    selected_feature = menu_screen_select_fea()
    while selected_feature in fea_ran_list:
      print('>>>Feature and its correlations are already displayed<<<')
      time.sleep(1)
      selected_feature = menu_screen_select_fea()
    new_data = create_print_rows(selected_feature, cleaned_correlation_matrix)
    fea_ran_list.append(selected_feature)
    exist_data = pd.concat([exist_data, new_data], axis=0)
    print(exist_data)
    time.sleep(1)
  else:
    print('>>>I hope you learned something from this program! Stay safe and healthy!')
    print('************************************************************')
    break
if l == 6:
  print('>>>All variables explores! I hope you learned something from this program! Stay safe and healthy!')
  print('************************************************************')
